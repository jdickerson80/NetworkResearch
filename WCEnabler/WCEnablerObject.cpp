...
//	system( "echo 2 > /sys/module/mptcp_ndiffports/parameters/num_subflows" );
	...
	// create the threads
	_bandwidthCalculator = new BandwidthCalculator( buildLogger( interface, "BandwidthCalculator", true ), interface, _ipAddress );
...
Common::LoggingHandler* WCEnablerObject::buildLogger( const std::string& interface, const std::string& filename, bool isCSV )
{
...
std::string WCEnablerObject::getInterfaceName()
{
...
void WCEnablerObject::setECNEnabled( bool isEnabled )
{
	if ( isEnabled )
	{
