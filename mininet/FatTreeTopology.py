...
class StructuredTopo(Topo):
	...
	def draw(self, filename = None, edge_width = 1, node_size = 1, node_color = 'g', edge_color = 'b'):
	...
	class FatTreeNodeID(NodeID):
		...
		def __init__(self, pod = 0, sw = 0, host = 0, dpid = None, name = None):
			...
			if dpid:
				...
				ten, pod, sw, host = [int(s) for s in name.split('.')]
				self.pod = pod
				self.sw = sw
				self.host = host
		...
		for p in pods:
			for e in edge_sws:
				...
				for h in hosts:
					...
					self.addLink(host_id, edge_id, cls=TCLink, enable_ecn=True, bw= speed * 1000 )
				...
				for a in agg_sws:
					...
					self.addLink(edge_id, agg_id, cls=TCLink, enable_ecn=True, bw=speed * 1000 )
			...
			for a in agg_sws:
				...
				for c in core_sws:
					...
					self.addLink(core_id, agg_id, cls=TCLink, enable_ecn=True, bw=speed * 1000 )
